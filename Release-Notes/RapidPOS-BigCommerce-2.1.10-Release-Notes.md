...
- This fix applies when the **Update UPC** setting is enabled.  
  - When Update UPC is enabled, the connector sends the ITEM barcode when available; otherwise it uses the first barcode in the barcode table (**IM_BARCOD**) linked to the item.  
