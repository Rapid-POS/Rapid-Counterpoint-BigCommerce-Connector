# BigCommerce Connector - GL Account Numbers
...
The BigCommerce connector creates a new BigCommerce store (**204**) in Counterpoint.

As part of the store setup, several general ledger (GL) accounts must be configured for BigCommerce-related activity, including:
...
- If you do not use an accounting integration, you can continue using standard Counterpoint reporting for all store activity, including the BigCommerce store.
...
- If you integrate Counterpoint with an accounting system, please review the default BigCommerce account numbers listed below.
...
>- If you are using profit centers, please notify Rapid prior to connector installation if you would like the Store 204 profit center set to something other than 204.
...
This account records amounts paid by customers on BigCommerce.
- BigCommerce payments are imported into Counterpoint using the generic tender of `EC_BIGCOMM`.
- This pay code is recorded in the order header of imported BigCommerce orders to indicate payment was collected on BigCommerce.

**Note:** The HOME currency code is used by default. If a different currency is preferred, please notify Rapid staff.

---

## BigCommerce Deposit Liabilities

| Field | Value |
|---|---|
| Default Account Number | 2254 |
...
When a customer makes a payment (deposit) on a BigCommerce order:
- the BigCommerce Tender account (`EC_BIGCOMM`) is debited
- the BigCommerce Deposit Liabilities account is credited

This liability is relieved when:
1. the order is released in Counterpoint, and
2. the associated BigCommerce drawer is posted.

---

## BigCommerce Sales Tax Payable

| Field | Value |
|---|---|
| Default Account Number | 2314 |
...
This account records tax amounts collected from customers on BigCommerce.
- BigCommerce controls all tax calculation and reporting.
- An `EC_BIGCOMM` Tax Authority and Tax Rule are automatically created so tax amounts flow into Counterpoint exactly as charged on BigCommerce.
...
- It records shipping fees collected from BigCommerce customers.
...
## BigCommerce Needs Investigation
...
| Field | Value |
|---|---|
| Default Account Number | 9904 |
| Label | BigCommerce Needs Investigation |
...
Counterpoint requires all possible store activity types to be assigned to accounts, even for activity types not used by the BigCommerce connector.

Rapid recommends using a shared catch all account of **9904 - BigCommerce Needs Investigation**. Most clients use this single investigation account rather than creating separate accounts for each unused activity type.
...
The default account configurations outlined in this guide are designed for standard BigCommerce connector implementations within Counterpoint (using Rapid's standard chart of accounts).
